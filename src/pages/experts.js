    ...
    <Fade>
	    <PortfolioContainer>
			...
			<SmallWrapper>
				...
				<p>There were many stakeholder constraints in this project that came together to shape many of the final design decisions. Primarily, my designs had to account for an engineering team with low bandwidth. To ease use of engineering resources, I had to repurpose many components from across wikiHow and stick to pre built wikiHow technologies to build this new custom page.</p>

				<HalfWidthImage fluid={props.data.stakeholderimage.childImageSharp.fluid} alt="" />

				<p>In addition to engineering constraints, I also had to account for the Experts team, and design a system that would accommodate them having to manually fill all data.</p>

				<br></br>

				<h3>Solution</h3>
				<p>My final designs focus on authority by providing details about the expert in a resume style format.</p>
			</SmallWrapper>

			<FullWidthImage fluid={props.data.solutionimage1.childImageSharp.fluid} alt="" />

			<SmallWrapper>
				<p>At the top, I created a header with the experts name, photo, and job title to create visual consistency with the verification badge.</p>
				<HalfWidthImage fluid={props.data.solutionimage2.childImageSharp.fluid} alt="" />
				<p>Below this there are three specifically editable sections (“wikiText areas”) designed to ease engineering resources. These sections were made editable by using the same backend editing that the rest of our site uses.</p>
				<HalfWidthImage fluid={props.data.solutionimage3.childImageSharp.fluid} alt="" />
				<p>This allowed more flexibility in editing for the Experts team and less custom engineering from the developers. A tradeoff of using these sections however was the limited text formatting that came with it.</p>
				<br></br>
				...
				<h3>Context</h3>
				<p>To provide some context for the design process on this project - stakeholders at wikiHow generally like to see high fidelity iterations. Given wikiHow's design language and system, I will often jump quickly to a high fidelity stage and test and iterate from there.</p>
				<br></br>
				<p>In addition to plenty of stakeholder constraints mentioned above, wikiHow generally likes to move very quickly and often doesn't have much time for user research.</p>
				<br></br>
...
export const ImageQuery = graphql`
	query {
	  ...
	  stakeholderimage: file(relativePath: {eq: "expert/stakeholderimage.png"}) {
	    childImageSharp {
	      fluid (maxWidth:888) {
	        ...GatsbyImageSharpFluid
	      }
